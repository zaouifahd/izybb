...
class LikesVC: BaseViewController, FBInterstitialAdDelegate {
    ...
    var offset: Int?
    var likeList: [AppNotification] = []
    
    
    var itemInfo = IndicatorInfo(title: "View")
    
    //    var listLikes: [[String:Any]] = []
    ...
    private func appendNotification(with userList: [AppNotification], at atFirst: Bool) {
        ...
        case false:
            let firstIndex = likeList.count
            let newList = userList.compactMap { (user) -> AppNotification? in
                let isSameUser =  likeList.filter { $0.notifierID == user.notifierID }.first
            ...
            likeList.append(contentsOf: newList)
...
extension LikesVC:UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.notification = likeList[indexPath.row]
        cell.notifyTypeIcon.image = UIImage(named: "ic_likes_notification")
        return cell
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let user = likeList[indexPath.row].notifierUser
...
extension LikesVC:IndicatorInfoProvider{
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        
        return IndicatorInfo(title: NSLocalizedString("Likes", comment: "Likes"))
    }
}
