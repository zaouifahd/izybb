...
import Async
class MapController: BaseViewController,CLLocationManagerDelegate,GMSMapViewDelegate,UITextViewDelegate{
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
      //  self.view.backgroundColor = .Main_StartColor
       // self.searchView.backgroundColor =  .Main_StartColor
        self.myLocation.setTitleColor(UIColor.hexStringToUIColor(hex: "FF007F"), for: .normal)
        self.nearByBtn.setTitleColor(UIColor.hexStringToUIColor(hex: "FF007F"), for: .normal)
        self.myLocation.tintColor = UIColor.hexStringToUIColor(hex: "FF007F")
        self.nearByBtn.tintColor = UIColor.hexStringToUIColor(hex: "FF007F")
        self.doneBtn.backgroundColor = UIColor.hexStringToUIColor(hex: "FF007F")
...
//        self.mapView.settings.myLocationButton = true
    ...
    func createCircleView() {
     ...
     func GetAddress1(Lat:Double,Long:Double){
        ...
        GoogleGeoCodeManager.sharedInstance.geoCode(type: "latlng", lat: Lat, lng: Long) { (success, authError, error) in
            if (success != nil){
...
func reverseGeoCode(address:String){
    ...
    GoogleGeoCodeManager.sharedInstance.reverseGeoCode(address: address) { (success, authError, error) in
        if (success != nil){
        ...
        //                    self.marker?.title = "Hello World"
            ...
            self.dismissProgressDialog {
                print(authError?.error_message ?? "")
                self.view.makeToast(authError?.error_message ?? "")
            }
        }
        else if error != nil{
            self.dismissProgressDialog {
